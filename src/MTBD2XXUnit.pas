...

{
  ...
  This unit is basically just an interface to FTDI ftd2xx.dll communication
  library. MTBusb.pas calls functions in this file to communicate with MTB-USB
  module.
  Installation of ftd2xx.dll (drivers from FTDI generally) is needed.
...
Begin
Result := ''; I := 1;
FT_Device_String_Buffer[50] := 0; // Just in case !
While FT_Device_String_Buffer[I] <> 0 do
  Begin
  Result := Result + chr(FT_Device_String_Buffer[I]);
  Inc(I);
  End;
End;
...
Var I,L : Integer;
Begin
  FT_Device_String_Buffer[1] := 0;
  L := Length(S);  If L > 49 then L := 49;
  If L = 0 then Exit;
  For I := 1 to L do FT_Device_String_Buffer[I] := ord(S[I]);
  FT_Device_String_Buffer[L+1] := 0;
End;
